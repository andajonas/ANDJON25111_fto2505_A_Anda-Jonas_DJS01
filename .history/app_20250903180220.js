const podcasts = [
    {
      ...
      description: "Weekly deep dive into technology trends, AI, and startups.",
      seasonList: [
  ...
  function openModal(p) {
    ...
    modalBody.innerHTML = `
      ...
      <p>${p.description}</p>
      <div class="tags">${p.genres.map(g => `<span class="tag">${g}</span>`).join("")}</div>
      <p><b>Last updated:</b> ${new Date(p.updated).toDateString()}</p>
      <h3>Seasons</h3>
      <ul>
        ${p.seasonList.map(s => `<li><b>${s.name}</b> - ${s.desc} (${s.episodes} episodes)</li>`).join("")}
      </ul>
    `;
  }
